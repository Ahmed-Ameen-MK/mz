...
// ===========================================================
// اللائحة النهائية للبطولات المسموح عرضها — أي بطولة مش موجودة هنا
// (بطولات نسائية، ناشئين، دوريات ثانوية...) تتفلتر تلقائيًا ومتظهرش
// إطلاقًا، بدل الاعتماد على "أولوية عرض" فقط. رقم أقل = يظهر أولًا.
//
// ⚠️ تأكد من رقمي 5 و6 و7 و301 فعليًا عبر:
// https://v3.football.api-sports.io/leagues?search=... (بمفتاحك)
// فهذول الأربعة (دوري الأمم الأوروبية، كاس أمم أفريقيا، كاس آسيا،
// الدوري الإماراتي) مش نفس أرقام الدوريات الأوروبية الكبرى اللي
// كانت مُتحقق منها قبل كده في الكود، والـ API أحيانًا بيغيّر ترقيم
// بعض البطولات القارية.
// ===========================================================
const ALLOWED_LEAGUES = {
  ...
  301: 5,  // الدوري الإماراتي (دوري الخليج العربي) — تحقق من الرقم
  233: 6,  // الدوري المصري الممتاز
  5: 7,    // دوري الأمم الأوروبية — تحقق من الرقم
  6: 8,    // كاس الأمم الأفريقية — تحقق من الرقم
  7: 9,    // كاس آسيا (كاس الأمم الآسيوية) — تحقق من الرقم
};
// اسم قديم محتفظ بيه بالتوافق مع باقي الكود بالأسفل (نفس المرجع)
const MAJOR_LEAGUES = ALLOWED_LEAGUES;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchFixtures(day) {
    ...
    // فلترة فورية: أي مباراة ببطولة مش موجودة في ALLOWED_LEAGUES
    // (نسائي، ناشئين، دوريات ثانوية...) بتتشال من الأساس ومتوصلش
    // للعرض إطلاقًا.
    return (data.response || []).filter(
      fx => ALLOWED_LEAGUES[fx.league.id] !== undefined
    );
  }
